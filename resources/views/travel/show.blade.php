...
@section('title', 'Travel Requisition Detail')
...
<div class="row">
        ...
        <div>
            ...
            <table class="table table-bordered">
                <tr>
                    ...
                    <td class="pb-1 pt-1">
                        <span class="text-danger"><strong>{!! getTravelCurrentLocation($trails, $delegations, $canceled) !!}</strong></span>
                        <span
                            class="badge-pill badge-success float-right text-white">{{$travel->requisitionStatus->name}}
                        </span>
                    </td>
                </tr>
            </table>
                                ...
                                <div class="row">
                                    <table class="table">
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $travel->vehicleType->name ?? 'Pending' }}</td>
        ...
        <div class="mt-3 mb-3">
            ...
            <span data-toggle="tooltip" data-placement="top" title="Approve and send to the next office"
                ...
                <a href="javascript:void(0);" class='btn btn-lg btn-warning text-white' data-toggle='modal'
                    data-target='#confirm_travel'> <i class='fa fa-check'></i> Approve Travel </a>
            </span>
            @endif
    ...
    <adiv class="col-xl-3 col-lg-4">
        ...
        <table class="table table-bordered table-striped trailTable">
            <thead>
                <tr>
                    <th colspan="3">Transport - <a href="{{ route('travel/show', $travel->id) }}">{{
                            idFormatter('transport', $transport->id) }}</a></th>
                </tr>
            </thead>
            <tbody>
                <?php
            $transportTrails = $transport->trails()->get();
            $i = 1;
        ...
        <table class="table table-bordered table-striped trailTable">
            <thead>
                <tr>
                    <th colspan="3">Subsistence - <a href="{{ route('travel/show', $travel->id) }}">{{
...
@if ($ableTo['submit'])
@include('travel.feedback.submit_modal')
@endif
@if ($ableTo['travel'])
@include('travel.feedback.travel_modal')
@endif
@if ($ableTo['return'])
@include('travel.feedback.return_modal')
@endif
@if ($ableTo['delegate'])
@include('travel.feedback.delegate_modal')
@endif
@if ($ableTo['delete'])
@include('travel.feedback.delete_modal')
@endif
@if ($ableTo['cancel'])
@include('travel.feedback.cancel_modal')
@endif
@if ($ableTo['finishDelegate'])
@include('travel.feedback.finish_delegate_modal')
@endif
@if ($ableTo['changeOwner'])
@include('travel.feedback.change_owner_modal')
@endif
@if ($ableTo['archive'] || $ableTo['unarchive'])
@include('travel.feedback.archive_modal')
@endif
