<table class="table table-bordered table-hover">
    ...
    <tbody>
        ...
        @foreach($requisitions as $subsistence)
        <tr>
            <td >{{ idFormatter('subsistence', $subsistence->id) }}</td>
            <td >
                @if ($subsistence->travel->procurement->archived)
                    <i class="fa fa-file-archive"></i>
                @endif
                {{ idFormatter('procurement', $subsistence->travel->procurement_id) }}
            </td>
            <td ><a href="{{ route('subsistence/show', $subsistence->id) }}">{{ $subsistence->travel->procurement->title }}</a></td>
            <td >{{ $subsistence->travel->procurement->createdUser->name }}</td>
            <td >{{ $subsistence->travel->procurement->unit->category }}</td>
            <td >{{ $subsistence->travel->procurement->unit->name }}</td>
            <td >{{ $subsistence->requisitionStatus->name }}</td>
            <td >{{ dateformat($subsistence->created_at) }}</td>
            <td >{{ dateformat($subsistence->updated_at) }}</td>
            <td >{{ empty($subsistence->currentUser) ? '-' : $subsistence->currentUser->name }}</td>
            <td >
                <div class="btn-group" role="group">
                    <a href="{{ route('subsistence/show', $subsistence->id) }}" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i></a>
                </div>
            </td>
