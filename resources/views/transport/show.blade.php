...
@section('title', 'Transport Requisition Details')
@section('top-content')
...
<div class="row">
    <div class="col-xl-9 col-lg-8 " style="padding-bottom: 15px;">
        ...
        <div>
            ...
            <table class="table table-bordered">
                <tr>
                    ...
                    <td class="pb-1 pt-1">
                        <span class="text-danger"><strong>{!! getTransportCurrentLocation($transportTrails, $delegations,
                                $canceled) !!}</strong></span>
                        <span
                            class="badge-pill badge-success float-right text-white">{{$transport->requisitionStatus->name}}</span>
                    </td>
                </tr>
            </table>
            ...
            <ul class="nav nav-tabs">
                ...
                <li class="nav-item"><a data-toggle="tab" class="nav-link {{-- $activeQuotationTab?'':'active' --}}"
                        href="#docs-card"><strong>Supporting documents</strong> <span
                            class="badge badge-{{ $documents->isEmpty()?'secondary':'primary' }}">
                                ...
                                <div class="row">
                                    <table class="table">
                                        ...
                                        <tr>
                                            ...
                                            @if(is_null($travel->vehicle_type_id))
                                            <td>Not a department vehicle</td>
        ...
        <div class="card mt-3 b-0 border">
            <div class="table-responsive">
                <table class="table table-bordered table-hover">
                    <tr>
                        <th class="text-center h4" colspan="6">List of travellers</th>
                    </tr>
                    <tr>
                        <th>Name</th>
                        <th>Department</th>
        ...
        <div class="mt-3 mb-3">
            ...
            <!-- // --- Right side ------------------------------------------------- -->
            @if ($travel->driver_id && $travel->vehicle_id && $ableTo['submit'])
            <!-- Submit button -->
                <span data-toggle="tooltip" data-placement="top" title="Approve and send to the next office"
                    class="float-right ml-2">
                        <a href="javascript:void(0);" class='btn btn-lg btn-warning text-white' data-toggle='modal'
                            data-target='#confirm_submit'> <i class='fa fa-check'></i> Submit
                        </a>
               </span>
            @endif
            <!-- // --- Right side ------------------------------------------------- -->
            @if ($ableTo['allocateDriver'])
            <!-- Submit button -->
            <span data-toggle="tooltip" data-placement="top" title="Allocate vehicle and driver"
                class="float-right ml-2">
                <a href="javascript:void(0);" class='btn btn-lg btn-primary text-white' data-toggle='modal'
                    data-target='#allocate_vehicle_modal'> <i class="fas fa-car"></i> {{ !$travel->driver_id ? 'Allocate driver' : 'Re-allocate driver' }} </a>
            </span>
            @endif
            ...
            <span data-toggle="tooltip" data-placement="top" title="Send back to one who delegated"
                class="float-right ml-2">
                <a href="javascript:void(0);" class="btn btn-lg btn-warning text-white" data-toggle='modal'
                    data-target='#approve_transport'><i class='fa fa-check'></i> Approve </a>
            </span>
            @endif
            @if ($ableTo['return'])
            <!-- Return button -->
    ...
    <div class="col-xl-3 col-lg-4">
        ...
        @endif
        <a href="{{ route('downloadTransport-pdf', $transport->id) }}" class="text-white btn btn-warning"> Download </a>
    </div>
...
@if ($ableTo['submit'])
@include('transport.feedback.submit_modal')
@endif
@if ($ableTo['transport'])
@include('transport.feedback.transport_modal')
@endif
@if ($ableTo['return'])
@include('transport.feedback.return_modal')
@endif
@if ($ableTo['delegate'])
@include('transport.feedback.delegate_modal')
@endif
@if ($ableTo['delete'])
@include('transport.feedback.delete_modal')
@endif
@if ($ableTo['cancel'])
@include('transport.feedback.cancel_modal')
@endif
@if ($ableTo['finishDelegate'])
@include('transport.feedback.finish_delegate_modal')
@endif
@if ($ableTo['changeOwner'])
@include('transport.feedback.change_owner_modal')
@endif
@if ($ableTo['archive'] || $ableTo['unarchive'])
@include('transport.feedback.archive_modal')
@endif
@if ($ableTo['allocateDriver'] )
@include('transport.feedback.allocate_vehicle_modal')
@endif
